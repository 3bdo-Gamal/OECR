...
class PaddleOCREngine(OCREngine):
    ...
    def extract(self, image_path: str, languages: Optional[List[str]] = None,
        ...
        try:
                    ...
                    for region in regions:
                        if isinstance(region, dict) and region.get('type') == 'Table':
                            ...
                            if bbox:
                                ...
                                # Blank out the table region (white)
                                cv2.rectangle(orig_img, (x1, y1), (x2, y2), (255, 255, 255), -1)
                except Exception as e:
                    logger.warning(f"Failed to use PPStructure for table skipping: {e}")
            ...
            full_text = _postprocess_text(full_text).strip()
            
            # Populate OCRResult fields
            lines = []
            conf_sum = 0.0
            for item in ordered_boxes:
                ...
                lines.append(OCRLine(
                    ...
                    language="ar" if _is_arabic_text(item["text"]) else "en"
                ))
                
            avg_conf = conf_sum / len(ordered_boxes) if ordered_boxes else 0.0
            ...
            return OCRResult(
                ...
                confidence=avg_conf,
                engine=self.name,
                language=get_setting("ocr_lang", "ar"),
                layout_data=ordered_boxes
            )
            
        except Exception as e:
            logger.error(f"PaddleOCR Engine Error: {str(e)}")
            return self._make_empty_result()
